/*
 * Name			:	changeLookupField
 Change History
 **********************************************************************************************************
 Modified By			Date			Jira No.		Description							Tag
 **********************************************************************************************************
Isha Bansal		27/03/2023		I2RT-6727		 Passed sobjectAPi name  for changeowner		T01
 */
...
export default class Lookup extends NavigationMixin(LightningElement) {
	...
	//API variables.
	@api strCustomNoRecordsFoundMessage;

	//Private variables.
	strNoRecordsFoundMessage = "No results.";
    ...
    @api sobjectapiname; //T01
    @api isusersearchonly=false; //T01
    // Template properties
	...
	connectedCallback() {
		...
		//If we received a custom No Records Found message, we display it.
		if(objUtilities.isNotBlank(this.strCustomNoRecordsFoundMessage)) {
			this.strNoRecordsFoundMessage = this.strCustomNoRecordsFoundMessage;
		}
	}
    ...
    @api
    setSearchResults(results) { //called from parent component
        // Reset the spinner
        console.log('--inside search result --'+JSON.stringify(results));
        this.loading = false;
        ...
        this._searchThrottlingTimeout = setTimeout(() => {
            ...
            if (this._cleanSearchTerm.length >= this.minSearchTermLength) {
                ...
                const searchEvent = new CustomEvent('search', {
                    detail: { // T01 - sobjectapiname I2RT 6727
                        searchTerm: this._cleanSearchTerm,
                        rawSearchTerm: newSearchTerm,
                        selectedIds: this._curSelection.map((element) => element.id),
                        sobjectapiname: this.sobjectapiname?this.sobjectapiname:'Case',
                        isusersearchonly:this.isusersearchonly
                    }
                });
    ...
    // EVENT HANDLING
    handleInput(event) {
        console.log('--handleinput customlookup'+event.target.value);
        // Prevent action if selection is not allowed
        if (!this.isSelectionAllowed()) {
            return;
        }
        console.log('---calling updateSearchTerm--');
    ...
    handleFocus() {
        ...
        //to Load some values on focus to input
        const loadDefaultEvent = new CustomEvent('loaddefault', {
            detail: {
                loadDefault: true
            }
        });
        ...
        this.dispatchEvent(loadDefaultEvent);
	...
	/*
	 Method Name : setNoRecordsFoundMessage
	 Description : This method sets the custom No Records Found message.
	 Parameters	 : String, called from setNoRecordsFoundMessage, strNoRecordsFoundMessage Message.
	 Return Type : None
	 */
	@api
    setNoRecordsFoundMessage(strNoRecordsFoundMessage) {
        this.strNoRecordsFoundMessage = strNoRecordsFoundMessage;
    }
}