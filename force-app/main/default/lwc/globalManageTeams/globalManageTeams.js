 ...
 * Description	:	Global Manage Teams.
...
//Apex Controllers.
import getRecords from "@salesforce/apex/GlobalManageTeamsController.getRecords";
import getRecordsDeleted from "@salesforce/apex/GlobalManageTeamsController.getRecordsDeleted";
import getRecordsRelated from "@salesforce/apex/GlobalManageTeamsController.getRecordsRelated";
import getRecordsUpdated from "@salesforce/apex/GlobalManageTeamsController.getRecordsUpdated";

//Custom Labels.
import Refresh_Button from '@salesforce/label/c.Refresh_Button';
import Manage_Teams_Title from '@salesforce/label/c.Manage_Teams_Title';
...
//Class body.
export default class globalManageTeams extends NavigationMixin(LightningElement) {
	...
	objConfiguration = {
		...
		strCardTitle: Manage_Teams_Title,
		lstTabs: [
			{
	...
	renderedCallback() {
		if(!this.boolFinishedInitialLoad) {
			...
			this.template.querySelector('.customGeneralCSS').innerHTML = "<style> " + 
					"c-global-manage-teams lightning-card.wrapper > article > .slds-card__header {" + 
					"	display: none;" + 
					"} </style>";
		}
	}
				...
				if(objTab.strTabValue === strCurrentTab) {
					...
					if(objUtilities.isNotNull(objResult) && objUtilities.isNotNull(objResult.lstRecords) && objResult.lstRecords.length > 0) {
						objResult.lstRecords.forEach(objRecord => {
							if(objUtilities.isNotNull(objRecord.Team_Member__r) && objUtilities.isNotBlank(objRecord.Team_Member__r.Email)) {
								objParent.strClipboard += objRecord.Team_Member__r.Email + "; ";
							}
						});
					}
				}
	 ...
	 */
	 newRecord() {
        this[NavigationMixin.Navigate]({
            ...
            attributes: {
                objectApiName: "Engagement_Team__c",
                actionName: "new"
            },
            state : {
                nooverride: "1",
	...
	addRecords() {
		...
		objSource.forEach(objSelectedRecord => {
			lstRecords.push(objSelectedRecord.UserId);
		});
