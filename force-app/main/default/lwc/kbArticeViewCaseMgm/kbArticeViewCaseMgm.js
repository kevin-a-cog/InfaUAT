/*
   ...
   @Purpose          : Provide Case using search on KB Article View Page.
   @Testclass        :
   @JIRA             :
...
import SFDCHOST from '@salesforce/label/c.KB_Internal_Host';
...
export default class KbArticeViewCaseMgm extends LightningElement {
    @api recordId;
        ...
        try {
            ...
            switch (action.name) {
                case 'select':
                    ...
                    break;
                case 'view':
                    var varUrlToOpen = this.CASERECORDURL.replace('HOSTNAME', SFDCHOST).replace('RECORDID', row.sfid);
