...
import lookUp2 from '@salesforce/apex/RelinkOpptyController.lookUp';
import {
    ShowToastEvent
...
let FIELDS = ['Opportunity.Name', 'Opportunity.New_Org_Opportunity_Number__c'];

export default class LookupLwc extends LightningElement {

    @api valueId;
    @api defaultRecordTypeName = 'New Sales Opportunity';
    @api objName;
    @api iconName;
    @api labelName;
    @api readOnly = false;
    ...
    wiredObjectInfo({
        ...
        if (data) {
            ...
            if (recordTypeInfos.length > 1) {
                ...
                for (var i = 0; i < this.recordTypeOptions.length; i++) {
                    if (this.recordTypeOptions[i].label === this.defaultRecordTypeName) {
                        this.recordTypeId = this.recordTypeOptions[i].value;
                    }
                }
            } else {
    ...
    wiredRecords({
        ...
        if (data) {
            ...
            for (var i = 0; i < this.record.length; i++) {
                var elem = JSON.parse(JSON.stringify(this.record[i]));
                elem.Name = elem.Name.length > 50 ? elem.Name.substring(0, 50) + '...' : elem.Name;
                rec.push(elem);
            }
            this.options = rec;
        } else if (error) {
    ...
    wiredOptions({
        ...
        if (data) {
            this.record = data;
            this.error = undefined;
            this.valueObj = this.record.fields.New_Org_Opportunity_Number__c.value + ' - ';
            this.valueObj += this.record.fields.Name.value ? (this.record.fields.Name.value.length > 50 ? this.record.fields.Name.value.substring(0, 50) + '...' : this.record.fields.Name.value) : '';
