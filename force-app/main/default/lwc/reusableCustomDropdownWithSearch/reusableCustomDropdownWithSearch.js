...
@api setOptionVAlues(){
        //alert( this.optionsToDisplayAll);
        this.optionsToDisplayAll = (this.optionsAll && this.optionsAll.length > 0 ? this.optionsAll : []);
        this.optionsAll.forEach( (elemnt)=>{
            if(elemnt.isBlnSUpportAccFav){
                this.favSupportAcc.push(elemnt.AccConRel.Id);
            }
             
        });
        
        // alert(JSON.stringify( this.optionsToDisplayAll));
    ...
    @api setOptionsAndValues() {
       // alert(JSON.stringify(this.optionsAll));
    ...
    makefav(event){
        //alert('ci '+event.target.value);
        var ACrID=event.target.value;
        //this.favSupportAcc.forEach();
        this.dispatchEvent(new CustomEvent('myfav',{detail:event.target.value}));
        var tempSuppAccList =[];  
        //alert(JSON.stringify( this.optionsToDisplayAll) +' cccc ' );
        this.optionsAll.forEach(data=> {
            // alert(JSON.stringify( this.optionsToDisplayAll) +' cccc '+data.isBlnSUpportAccFav);
             let tempdata={...data};
            if(tempdata.AccConRel.Id==event.target.value && !tempdata.isBlnSUpportAccFav){
        ...
        //  alert(JSON.stringify( tempSuppAccList));
    ...
    filterDropdownList(key) {
       ...
       const filteredOptions = this.optionsAll.filter(item => item.AccConRel.Account.Name.toLowerCase().includes(key.toLowerCase())); 
    ...
    optionsClickHandler(event) {
        ...
        detail["value"] = value;
        detail["label"] = label;
        this.template.querySelector(`[data-id="${value}"]`).className = 'slds-listbox__item add-bg';
        this.dispatchEvent(new CustomEvent('change', { detail: detail }));
    }
    //Method to handle favicon
    ignoremakefav(event) {
        alert('ok');
