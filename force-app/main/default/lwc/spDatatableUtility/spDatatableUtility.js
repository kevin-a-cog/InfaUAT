...
export default class LwcDatatableUtilityTest extends LightningElement {
    ...
    @api hideCheckBox = false;//Show/hide Checkbox in the first row of data table
    //@api handleRowAction;// For Row action like edit/delete/add etc
    ...
    //     const detRow = event.detail.row;
    //     this.dispatchEvent(new CustomEvent('handleeditdelete', { row: detRow })); //Send records to display on table to the parent component
    // }
    handleRowAction(event) {
        const rowVal = event.detail.row;
        const actionVal = event.detail.action;
        console.log('Action'+JSON.stringify(event.detail.action));
        this.dispatchEvent(new CustomEvent('handleeditdelete', { detail: {rowValue:rowVal, actionValue:actionVal}})); //Send records to display on table to the parent component
    }
}